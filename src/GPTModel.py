...
def gpt2(payload):
    ...
    if (is_Key_set == False):
        VoiceEngine.say("Sorry I Can't UnderStand You . I Require A HuggingFace User Access Token To Work Properly . Do You Like To Know About It .")
        command = str(SpeechRecogniter.getinput()).lower()
        if "ok " in command or "sure" in command or "yes" in command : 
          notsetEnvDetails()
          return
        else :
          return     
...
def is_APIKey_Set(): 
  if "null" in huggingfacehub_api_token :
    return False
...
def notsetEnvDetails():
  ...
  command = str(SpeechRecogniter.getinput()).lower()
  if "ok" in command or "sure" in command or "do" in command or "open" in command or "yes" in command: webbrowser.open_new_tab(huggingface)
  else : return   
